...
def open_file():
    ...
    if file is None:  # Si el usuario no selecciona algún archivo
        ...
        for a in lineasSinComentario:
            constante = str(validadConstantesCaracter(a))
        ...
        for i in elements:
            ...
            if palabra.endswith('B') or palabra.endswith(('b')):  # CORREGIR CON FUNCION QUE VERIFIQUE QUE ES DECIMAL
                textoTotal += (palabra + '  ------> Constante Numerica Binaria' + '\n')
            ...
            if validarDUP(palabra):
                ...
                if validarComentario(
                        palabra) and palabra != '.data' and palabra != '.stack' and palabra != '.code' and palabra != 'segment':
                    textoElementos += (palabra + '\n')
                else:
                    pass
        textoElementos.strip()
